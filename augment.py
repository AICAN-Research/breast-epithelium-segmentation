...
# Augmentations
def random_brightness(x, brightness=25):  # look at second input parameter to random_brightness. How to deal with normalized (?)
    nbr = tf.random.uniform(shape=[], minval=0., maxval=1., dtype=tf.float32)
    x = tf.cond(nbr < 0.5, lambda: x, lambda: tf.clip_by_value(tf.image.random_brightness(x, brightness), 0, 255))
    return x
...
def random_hue(x, max_delta):
    nbr = tf.random.uniform(shape=[], minval=0., maxval=1., dtype=tf.float32)
    x = tf.cond(nbr < 0.5, lambda: x, lambda: tf.clip_by_value(tf.image.random_hue(x, max_delta=max_delta), 0, 255))
    return x

def random_saturation(x, saturation):
    nbr = tf.random.uniform(shape=[], minval=0., maxval=1., dtype=tf.float32)
    x = tf.cond(nbr < 0.5, lambda: x, lambda: tf.clip_by_value(tf.image.random_saturation(x, lower=1-saturation, upper=1+saturation), 0, 255))
    return x
