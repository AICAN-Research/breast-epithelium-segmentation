...
while True:
    ...
    if np.abs(dist_x) < dist_limit and np.abs(dist_y) < dist_limit:  # if positions are close we have a pair
        ...
        if plot_flag:
            fig, axes = plt.subplots(2, 3, figsize=(30, 30))  # Figure of TMAs
            axes[0, 0].imshow(mask[..., 0], cmap="gray", interpolation="none")
            # axes[0, 0].imshow(annot_TMA_padded[..., 0], alpha=0.5)
            axes[0, 1].imshow(gt_one_hot[..., 0], cmap="gray", interpolation="none", vmin=0, vmax=1)
            axes[0, 2].imshow(gt_one_hot[..., 1], cmap="gray", interpolation="none", vmin=0, vmax=1)
            axes[1, 1].imshow(gt_one_hot[..., 2], cmap="gray", interpolation="none", vmin=0, vmax=1)
            axes[1, 2].imshow(gt_one_hot[..., 3], cmap="gray", interpolation="none", vmin=0, vmax=1)
            plt.show()
        ...
        for patch_idx, (patch_HE, patch_mask, patch_healthy, patch_in_situ) in enumerate(zip(*streamers)):
            ...
            if (len(patch_HE) == 0) or (len(patch_mask) == 0):
                ...
                #print("shape patch CK", np.array(patch_mask).shape)
